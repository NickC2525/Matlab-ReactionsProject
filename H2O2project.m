...
Max_H2O2_out = max(C(:,1)) % find the most H2O2 production possible
k = find(C(:,1)==Max_H2O2_out); % get its array index
...
cat_amt = W(k) % mol
Production= Max_H2O2_out*34*vo*(1/1000)*3600*8400/1000 %tons/year
...
T = C(:,5); % Reactor Temperature

figure
plot(Wkg, T,'m');
title('Temperature vs Catalyst Weight'); xlabel('Weight [kgcat]');
